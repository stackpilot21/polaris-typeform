...
function ExecutiveSummarySection({ dealId }: { dealId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/deals/${dealId}/executive-summary`)
      .then((r) => r.json())
      .then((data) => {
      ...
      <CardHeader className="pb-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {generating ? (
                <div className="w-3 h-3 border-2 border-[#d8e3ef] border-t-[#0169B4] rounded-full animate-spin" />
              ) : (
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
